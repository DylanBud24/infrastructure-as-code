...
sync_skel() {
    # Copy /etc/skel/.* into $HOME — mostly non-destructive (existing user
    # files survive image upgrades) with a carve-out for known-autogenerated
    # defaults from first-boot apps (Hyprland writes a placeholder config the
    # moment it's launched without one, which would otherwise shadow ours).
    say "Syncing /etc/skel → \$HOME."
    local src="/etc/skel"
    [[ -d "$src" ]] || { warn "/etc/skel missing; skipping."; return; }

    # Files we always replace (autogenerated templates, never hand-edited).
    # If you customize these, the local edit wins only on subsequent runs
    # because they no longer match the autogenerated marker.
    local force_replace=(
        ".config/hypr/hyprland.conf"
    )
    local f
    for f in "${force_replace[@]}"; do
        local target="$HOME/$f"
        if [[ -f "$target" ]] && grep -q '^autogenerated = 1' "$target" 2>/dev/null; then
            say "  replacing autogenerated $f"
            mkdir -p "$(dirname "$target")"
            cp -f "$src/$f" "$target"
        fi
    done

    # Non-destructive copy for everything else.
